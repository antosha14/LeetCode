...
Другая частая сфера применения – это кеширование, когда результат вызова функции должен где-то запоминаться («кешироваться») 
`
...
Параметр space применяется исключительно для логирования и красивого вывода. и определяет кол-во пробелов
JSON.stringify вызывает встроенный метод объектов toJSON()
...
`
Object.keys(obj) Можно вызывать на любые объекты, возвращает реальный массив 
...
`
Выдаст ошибку так как подумает что первое это блок кода, чтобы не было ошибки нужно взять всё выражение в кавычки ()
